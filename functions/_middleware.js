...
// 将 Express 路由逻辑转换为 Web Request/Response API
...
function detectLanguage(request) {
  ...
  // 解析语言，返回第一个支持的语言
  // Accept-Language 格式通常是：zh-CN,zh;q=0.9,en;q=0.8
  ...
  // 检查是否有中文语言代码（zh-CN, zh-TW, zh-SG, zh, zh-HK 等）
...
// Cloudflare Pages Functions 导出的处理函数
export async function onRequest(context) {
  const { request } = context;
  const url = new URL(request.url);
  
  let code = parseInt(url.searchParams.get('code'), 10);
  const lang = detectLanguage(request);
  
  // 只有无效的状态码才默认转为 404，其他状态码（包括超出范围的）正常显示
  if (isNaN(code)) {
    code = 404;
  }
  ...
  // HTTP响应状态码必须在200-599之间才能正常渲染，100-199和600+都使用200
